export const portalDictionary = {
  general: [{
    subtitle: 'Введение',
    text: ['Фреймворк SolidJS был создан Райаном Карниато и выложен в открытый доступ в 2018 году, он приобрел популярность благодаря реализации “точечной реактивности” (fine-grained reactivity).', 'SolidJS разделяет почти все те же принципы, что и React, за некоторыми исключениями. Например, отсутствие виртуального DOM и отрисовка компонентов только один раз.', 'Эти функции способствуют невероятно высокой скорости, которой обладают приложения, созданные с помощью SolidJS, и делают его одним из самых быстрых фреймворков JavaScript, доступных сегодня.'],
  }],
  reactivity: [
    {
      subtitle: 'Signals',
